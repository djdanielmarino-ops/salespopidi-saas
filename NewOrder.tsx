...
export default function NewOrder() {
  ...
  const [items, setItems] = useState<OrderItemForm[]>([{ beer_type_id: '', barrel_model_id: '', barrel_volume: 30, barrel_quantity: 1, unit_price: 0 }]);
  ...
  const addItem = () => {
    setItems([...items, { beer_type_id: '', barrel_model_id: '', barrel_volume: 30, barrel_quantity: 1, unit_price: 0 }]);
  };
  ...
  const getItemTotal = (item: OrderItemForm) => {
    return item.barrel_volume * item.barrel_quantity * item.unit_price;
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!customerId || !deliveryDate || items.some(i => !i.beer_type_id || !i.barrel_quantity)) {
      return;
    }
    ...
    await createOrder.mutateAsync({
      ...
      items: items.filter(i => i.beer_type_id && i.barrel_quantity > 0).map(i => {
        ...
        return {
          ...
          quantity_liters: i.barrel_volume * i.barrel_quantity,
          unit_price: i.unit_price,
          barrel_quantity: i.barrel_quantity,
        };
      }),
    } as any);
            ...
            <CardContent>
              <div className="space-y-4">
                {items.map((item, index) => (
                  <div key={index} className="grid gap-4 md:grid-cols-5 items-end border-b pb-4 last:border-0">
                    <div>
                    ...
                    <div>
                      ...
                      <Input 
                        type="number" 
                        min="1"
                        value={item.barrel_quantity || ''}
                        onChange={(e) => updateItem(index, 'barrel_quantity', e.target.value)}
                      />
                    </div>
