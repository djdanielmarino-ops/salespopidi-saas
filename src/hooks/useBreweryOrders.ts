...
// Generated types predate the purchases migration.
...
export function useCreateBreweryOrder() { const qc=useQueryClient(); return useMutation({ mutationFn: async ({ items, ...order }: { supplier:string; expected_delivery_date?:string|null; notes?:string|null; items:Array<{barrel_model_id:string;beer_type_id:string;quantity_ordered:number;unit_cost?:number|null}> }) => { const { data, error }=await db.from('brewery_orders').insert(order).select('*').single(); if(error) throw error; const { error:itemError }=await db.from('brewery_order_items').insert(items.map(item=>({...item,brewery_order_id:data.id}))); if(itemError){ await db.from('brewery_orders').delete().eq('id',data.id); throw itemError; } return data; }, onSuccess:()=>{qc.invalidateQueries({queryKey:['brewery-orders']});toast.success('Pedido de compra criado.');}, onError:(e:Error)=>toast.error(e.message) }); }
